...
const UI = {
    ...
    renderBooksPage: async function () {
    try {
        const books = await Api.getBooks();

        // Crée le HTML avec le champ de recherche
        const html = `
            <h2>Liste des livres</h2>
            <input type="text" id="searchInput" placeholder="Rechercher un livre..." class="search-input">
            <div id="books-list">
                ${this.getBooksHtml(books)}
            </div>
        `;
        ...
        searchInput.addEventListener('input', async (e) => {
            ...
            const filteredBooks = await Api.searchBooks(query);
            document.getElementById('books-list').innerHTML = this.getBooksHtml(filteredBooks);
        });

    } catch (error) {
        this.showMessage("Erreur lors du chargement des livres", "error");
    }
},
